...
class CRM_Membership_Renewals_Email {
    ...
    $club_template = '11';
    $retailer_template = '10';

    // segment
    if ($segment == 'r') {
      $membership_type_ids = "1, 2, 3, 4, 5, 7, 8, 9, 10";
      /*
      Individual  1
      NMBP Patroller  2
      NMBP Ambassador  3
      ...
      Chapter  11
      */
      $sorba_ind_template = '12';
      $sorba_fam_template = '12';
      $imba_ind_template = '9';
      $imba_fam_template = '9';
      $nmbp_ind_template = '27';
      $nmbp_group_template = '30';
      $corporate_template = '32';
      $end_date_interval ="
AND    civicrm_membership.end_date >= date(now() - INTERVAL 2 MONTH)
AND    civicrm_membership.end_date < date(now() + INTERVAL 2 MONTH)";
    } else {
      $membership_type_ids = "1,5";
      $sorba_ind_template = '14';
      $sorba_fam_template = '14';
      $imba_ind_template = '14';
      $imba_fam_template = '14';
      $end_date_interval ="
AND    civicrm_membership.end_date >= date(now() - INTERVAL 18 MONTH)
AND    civicrm_membership.end_date < date(now() - INTERVAL 6 MONTH)";
    }
...
AND    civicrm_address.country_id != 1039
...
ORDER BY civicrm_contact.id";
    ...
    if (!$test) {
      ...
      if ( !$toEmail ) {
        ...
        $from = "membership@imba.com";

        $message_template = NULL;
        switch($allTypes[$dao->membership_type_id]) {
          case "Club":
          case "Promoter":
            $message_template = $club_template;
            break;
          case "Corporate":
            $message_template = $corporate_template;
            break;
          case "Retailer":
            $message_template = $retailer_template;
            break;
          case "NMBP Group":
            $message_template = $nmbp_group_template;
            break;
          case "NMBP Patroller":
          case "NMBP Ambassador":
            $message_template = $nmbp_ind_template;
            break;
          case "Individual":
            if ($dao->region == "sorba") {
              $message_template = $sorba_ind_template;
            } else {
              $message_template = $imba_ind_template;
            }
            break;
          case "Family":
            if ($dao->region == "sorba") {
              $message_template = $sorba_fam_template;
            } else {
              $message_template = $imba_fam_template;
            }
            break;
          default:
            $message_template = $imba_ind_template;
